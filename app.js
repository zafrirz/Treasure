...
const stableTargetUrl = row => `${SITE_URL}/${slugify(row.englishName) || row.slug}/`;
...
async function parseSelectedSheet() {
  ...
  for (let index = 0; index < objects.length; index += 1) {
    ...
    if (!HEADERS.some(header => asText(source[header]))) continue;
  ...
  const incomplete = parsedRows.filter(row => !row.riddle || !row.answerHash).length;
...
async function applyWorkbook() {
  ...
  try {
    ...
    const remoteFile = await github(`contents/${dataPath}?ref=${BRANCH}`, token, { allow404: true });
    const remoteConfig = remoteFile ? JSON.parse(base64ToText(remoteFile.content)) : structuredClone(currentConfig);
